...
module vars
    ...
    real(wp), parameter ::  lambda=2.96, ll=exp(lambda), epsi =  0.0001, alpha=0.3204, beta=2.75!0.000001 
    real(wp) :: valu,ul,percent
    real(wp), dimension(20) :: sbeta, salpha
    integer :: maxit, n, i
    !ll=lower limit
...
do i = -10,10
!write(*,'(A,I2,A,f5.3,f10.6,A)') "Beta=",i," ",sbeta(i+10),"Maximum design flow is ", max_flow, " (m^3/day)"
write(*,*) "Beta=",i," ",sbeta(i+10),"Maximum design flow is ", max_flow, " (m^3/day)"
...
function f(x)
    ...
    one = 1/(alpha*x*Gamma(sbeta(i+10)))
    two = ((log(x) - lambda)**(sbeta(i+10)-1))/(alpha**(sbeta(i+10)-1))
